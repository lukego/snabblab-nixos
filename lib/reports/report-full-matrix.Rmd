...
```{r}
ggplot_distribution(l2fwd0) + facet_wrap(qemu ~ dpdk, scales = "free_y", shrink = TRUE) + ggtitle("l2fwd by QEMU and DPDK")
```

### QEMU + DPDK + Config

```{r}
ggplot_distribution(l2fwd0) + facet_wrap(qemu ~ dpdk + config, scales = "free_y", shrink = TRUE) + ggtitle("l2fwd by QEMU and DPDK and config")
```
