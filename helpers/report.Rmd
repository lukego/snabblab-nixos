...
maxscore = 35 # maximum expect score
```
...
p <- p + geom_line()
p <- p + geom_point()
p <- p + scale_y_continuous(breaks = seq(from=0, to=maxscore, by=2))
...
```{r}
p <- ggplot(df, aes(y=score, x=benchmark, color=benchmark))
p <- p + geom_boxplot()
p <- p + scale_y_continuous(breaks = seq(from=0, to=maxscore, by=2))
p
```
...
p <- p + expand_limits(y=0) + expand_limits(y=maxscore)
p <- p + geom_violin()
